...
`doc/use_cases/README.md` contains a structured index (file table + keyword reverse index) at the top, followed by a narrative overview of use cases. The index is auto-generated by `doc/use_cases/generate-index.sh` from `#+SUBTITLE:` metadata in the org files and auto-detected clk API references in code blocks.
...
3. Add `#+SUBTITLE: one-line description with key terms` after `#+EXPORT_FILE_NAME:` (used by `generate-index.sh` to build the index in `README.md`)
4. Use `:session UUID` on all bash code blocks to share state
