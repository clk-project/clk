...
#+SUBTITLE: Using clk value to set semantic defaults across many commands, parameters vs values
...
As your configuration evolves, some values stop being useful. Say you set one
for an application you no longer work on:
...
#+BEGIN_SRC bash :exports none :tangle ../../tests/use_cases/setting_default_values.sh :noweb yes :shebang "#!/usr/bin/env bash"
  set -eu
<<init>>
