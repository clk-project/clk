...
Ask the same program about ~main~ and it complains just as much, yet gives you
an answer.
...
What it said reaches you as it said it, with nothing added, and the answer is
still yours to use.

A program may also do neither: not answer, not give up, simply keep you waiting.
Pass ~timeout~ to ~check_output~ and it will not wait beyond it. Half a minute is
long enough for a cluster, and an option lets you be less patient than that.
...
#+RESULTS[]: run-waiting-demo
: error: bash -c 'sleep 600' did not finish in 1s
: giving up, the cluster is not answering

clk says which program ran out of patience, and the ~TimeoutExpired~ is yours to
catch, so your command decides what to do about it.
