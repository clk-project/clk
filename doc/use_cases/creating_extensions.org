...
Each profile decides for itself whether an extension is enabled. Say you work on
a project that has nothing to do with kubernetes: you can disable k8s there, and
it remains available everywhere else.

#+NAME: create-noisy-project
#+BEGIN_SRC bash :results none :exports code :session f47428c2-56ea-4d91-97d9-4f2ff63ef052
mkdir noisy-project && cd noisy-project && mkdir .clk
#+END_SRC

#+NAME: disable-in-project
#+BEGIN_SRC bash :results verbatim :cache yes :exports both :session f47428c2-56ea-4d91-97d9-4f2ff63ef052
clk extension disable k8s
clk k8s run-dev-env --flow 2>&1 | tail -1
#+END_SRC

#+RESULTS[]: disable-in-project
: error: No such command 'k8s'.

Now this project starts using a cluster as well, and you want it to behave like
everywhere else. Rather than enabling the extension here, you can remove what
this project decided, with ~clk extension unset~. The project decides nothing
about k8s anymore, so your global choice applies again.

#+NAME: unset-in-project
#+BEGIN_SRC bash :results verbatim :cache yes :exports both :session f47428c2-56ea-4d91-97d9-4f2ff63ef052
clk extension unset k8s
clk k8s run-dev-env --flow
#+END_SRC

#+RESULTS[]: unset-in-project
: installing dependencies
: starting k8s cluster
: starting controllers
: noop, this must be overloaded by a project command
...
* generic programming
  ...
  Here we deploy differently, and we shadow the command with one of our own,
  keeping the flow of the one we hide.

  #+NAME: create_project_deploy
  #+BEGIN_SRC bash :results none :exports code :session f47428c2-56ea-4d91-97d9-4f2ff63ef052
  clk command create bash deploy --flowdeps '[overridden]' --description "Deploy the stack our way" --body 'echo deploying our way'
  #+END_SRC

  #+NAME: run_project_deploy
  #+BEGIN_SRC bash :results verbatim :cache yes :exports both :session f47428c2-56ea-4d91-97d9-4f2ff63ef052
  clk deploy
  #+END_SRC

  #+RESULTS[]: run_project_deploy
  : deploying our way

  The day the one we hide grows a flow, ours runs it too.

  #+NAME: set_deploy_flow
  #+BEGIN_SRC bash :results none :exports code :session f47428c2-56ea-4d91-97d9-4f2ff63ef052
  clk flowdep --extension k8s set deploy k8s.run-cluster
  #+END_SRC

  #+NAME: run_project_deploy_again
  #+BEGIN_SRC bash :results verbatim :cache yes :exports both :session f47428c2-56ea-4d91-97d9-4f2ff63ef052
  clk deploy --flow
  #+END_SRC

  #+RESULTS[]: run_project_deploy_again
  : installing dependencies
  : starting k8s cluster
  : deploying our way
...
So far that extension is yours alone. There is nowhere to pull it from, and clk
says so rather than trying.

#+NAME: update-before-publishing
#+BEGIN_SRC bash :results verbatim :cache yes :exports both :session f47428c2-56ea-4d91-97d9-4f2ff63ef052
clk extension update k8s 2>&1 | tail -1
#+END_SRC

#+RESULTS[]: update-before-publishing
...
Run that same line again, as one does when following a README, and clk pulls
rather than complains.

#+NAME: install-k8s-again
#+BEGIN_SRC bash :results verbatim :cache yes :exports both :session f47428c2-56ea-4d91-97d9-4f2ff63ef052
clk extension install "${TMP}/k8s.git" k8s 2>&1 | tail -1
#+END_SRC

#+RESULTS[]: install-k8s-again
...
Working on the extension yourself, you end up on a branch of your own, with
...
Update now and clk says where you are before pulling, putting your work aside
while it does.

#+NAME: update-while-working
#+BEGIN_SRC bash :results verbatim :exports both :session f47428c2-56ea-4d91-97d9-4f2ff63ef052 :cache yes
clk extension update k8s --stash 2>&1 | head -1
#+END_SRC

#+RESULTS[]: update-while-working
...
<<create-noisy-project>>

check-result(disable-in-project)

check-result(unset-in-project)
...
<<create_project_deploy>>

check-result(run_project_deploy)

<<set_deploy_flow>>

check-result(run_project_deploy_again)
...
check-result(update-before-publishing)
...
check-result(install-k8s-again)

<<another-k8s>>

check-result(refuse-another-k8s)

check-result(install-from-nowhere)
...
check-result(update-while-working)
