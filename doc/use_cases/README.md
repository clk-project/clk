...
Most of my bash commands start with simple aliases (see [aliases](#aliases)). You can read more about this way of thinking in [controlling my music](controlling_my_music.md). The [standard library](#standard-library) section covers the built-in helpers available in bash commands.
...
-   [cloud provider CLI wrapper](wrapping_a_cloud_provider_cli.md) &ndash; global parameters set a default AWS profile, but local parameters in a project directory override them. When you leave the project, the global defaults take over again. Extensions named `config-*` override global settings, so enabling a `config-staging` extension [switches the whole CLI context](wrapping_a_cloud_provider_cli.md) without touching global configuration.
...
clk does not want to compete with dedicated flow tools like nodered, but it helps having basic flow handling, like when you have a [3D printing flow](3D_printing_flow.md). The [chaotic simulator manager](chaotic_simulator_manager.md) also uses flows to chain a generate → configure → build → simulate pipeline in a standalone tool. The [end-to-end example](#end-to-end-example) combines flows with [parameters](#parameters-and-configuration), [aliases](#aliases), and [projects](#projects).
...
To distribute your commands as installable packages, see [how to create your own extensions](creating_extensions.md). Extensions can be shared across [projects](#projects) and follow the same [resolution priority](#resolution-priority) rules. They can also be used to [override global parameters](wrapping_a_cloud_provider_cli.md), making it easy to toggle between sets of settings (e.g. staging vs production). For deeper customisation, see [plugins](#plugins).
...
If you don't want to use the clk command line tool, you can [roll your own](chaotic_simulator_manager.md). The example builds a standalone `csm` tool using `clk fork` and shows how to grow it with your own customizations: auto-discovered internal commands, [flow](#flows) dependencies to chain them, and a custom launcher mechanism that wraps commands with tools like gdb, valgrind, or perf.
...
- `A:` : [bash_command.md](bash_command.md), [bash_command_use_option.md](bash_command_use_option.md), [controlling_my_music.md](controlling_my_music.md), [finding_recent_documents.md](finding_recent_documents.md), [ipfs_name_publish.md](ipfs_name_publish.md), [send_sms.md](send_sms.md)
...
- `CLK___NAME` : [bash_command.md](bash_command.md)
