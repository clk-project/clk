...
#+SUBTITLE: Restricting user input to predefined values using Choice types
...
* when the value is made of several parts
  :PROPERTIES:
  :CUSTOM_ID: when-the-value-is-made-of-several-parts
  :END:

  A value is not always a single word. The style of ~clk echo~ is a colour, or a
  colour on a background, or a colour in bold. Completion follows you through it,
  a part at a time.

  #+NAME: style-to-complete
  #+BEGIN_SRC bash :results none :exports none
    echo --style
  #+END_SRC
  ...
  #+NAME: ask-for-style
  #+BEGIN_SRC bash :results none :exports code :noweb yes
    clk <<style-to-complete>> <TAB>
  #+END_SRC

  #+NAME: try-style
  #+BEGIN_SRC bash :results verbatim :exports results :noweb yes :cache yes :session a7b996e6-a3e6-410e-a91b-311640c4ada8
    clk completion try --last <<style-to-complete>> "" | head -4
  #+END_SRC
  ...
  #+NAME: ask-for-foreground
  #+BEGIN_SRC bash :results none :exports code :noweb yes
    clk <<style-to-complete>> fg-<TAB>
  #+END_SRC

  #+NAME: try-foreground
  #+BEGIN_SRC bash :results verbatim :exports results :noweb yes :cache yes :session a7b996e6-a3e6-410e-a91b-311640c4ada8
    clk completion try --last <<style-to-complete>> "fg-" | head -3
  #+END_SRC

  #+RESULTS[]: try-foreground
  : fg-black
  : fg-red
  : fg-green

  Nothing stops you typing past the completion, so ask for a colour it never
  offered.

  #+NAME: try-unknown-colour
  #+BEGIN_SRC bash :results verbatim :exports both :session a7b996e6-a3e6-410e-a91b-311640c4ada8 :cache yes
    clk echo --style pink hello
  #+END_SRC

  #+RESULTS[]: try-unknown-colour
  : Usage: clk echo [OPTIONS] [MESSAGE]...
  : error: Invalid value for '-s' / '--style': invalid style: pink. (Unknown color 'pink')

  clk turns it down before the command runs. Get the other half wrong and it
  lists the parts a style is made of.

  #+NAME: try-unknown-part
  #+BEGIN_SRC bash :results verbatim :exports both :session a7b996e6-a3e6-410e-a91b-311640c4ada8 :cache yes
    clk echo --style wiggle-True hello
  #+END_SRC

  #+RESULTS[]: try-unknown-part
  : Usage: clk echo [OPTIONS] [MESSAGE]...
  ...
  #+NAME: echo-with-style-shown
  #+BEGIN_SRC bash :results none :exports code :session a7b996e6-a3e6-410e-a91b-311640c4ada8
    clk echo --style bold-True,fg-green hello
  #+END_SRC

  #+NAME: echo-with-style
  #+BEGIN_SRC bash :results verbatim :exports results :session a7b996e6-a3e6-410e-a91b-311640c4ada8 :cache yes
    clk --force-color echo --style bold-True,fg-green hello
  #+END_SRC

  #+RESULTS[]: echo-with-style
  : [32m[1mhello[0m

  #+NAME: run
  #+BEGIN_SRC bash :results none :exports none :tangle ../../tests/use_cases/choices.sh :noweb yes :shebang "#!/usr/bin/env bash"
    set -eu
    <<init>>
    <<export-with-choice>>
    ...
    check-result(try-style)
    check-result(try-foreground)
    check-result(try-unknown-colour)
    check-result(try-unknown-part)
    <<echo-with-style-shown>>
    check-result(echo-with-style)
  #+END_SRC
