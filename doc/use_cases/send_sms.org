...
the bash commands are put. Any file you put in that ~lib~ folder can be loaded
with ~clk_import~, which is how several commands end up sharing the same code.
...
#+NAME: run
#+BEGIN_SRC bash :results none :exports none :tangle ../../tests/use_cases/send_sms.sh :noweb yes :shebang "#!/usr/bin/env bash"
  set -eu
  <<init>>
  <<install-lib>>
  <<create-command>>
