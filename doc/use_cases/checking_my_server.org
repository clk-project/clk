...
#+NAME: install-fake-journalctl
#+BEGIN_SRC bash :results none :exports none :session 85c8e385-7f24-48ac-9a85-30cfc354aebf :noweb yes
  mkdir -p "${TMP}/bin"
...
#+RESULTS[]: logs-myserver
: myserver: started
: myserver: listening on 443

Let's ask for a unit that is not there. journalctl fails, and clk hands me its
exit code.
...
#+RESULTS[]: logs-nowhere
: Failed to get unit: Unit nowhere.service not loaded.
: error: journalctl --unit nowhere exited with 4
: 4

Read in a loop, I'd rather it kept quiet, and ~safe_check_output~ answers the
empty string instead.

#+NAME: logs-never-mind
#+BEGIN_SRC bash :results verbatim :exports both :session 85c8e385-7f24-48ac-9a85-30cfc354aebf :cache yes
clk server logs --never-mind nowhere
#+END_SRC

#+RESULTS[]: logs-never-mind
: no news
...
keeping it open. Let's make sure nothing is on that port already.
...
Whatever I do next, that tunnel has to go, so the command puts it in a trap, and
clk will try to respect it.
...
#+NAME: run-broken-logs
#+BEGIN_SRC bash :results verbatim :exports both :session 85c8e385-7f24-48ac-9a85-30cfc354aebf :cache yes
clk server logs myserver 2>&1
#+END_SRC

#+RESULTS[]: run-broken-logs
...
<<install-fake-journalctl>>

<<create-logs>>

check-result(logs-myserver)

check-result(logs-nowhere)

check-result(logs-never-mind)

<<create-port>>

check-result(run-port)
...
check-result(run-broken-logs)

check-result(run-with-report-file)
#+END_SRC
