...
* Adding values in the created class
  :PROPERTIES:
  :CUSTOM_ID: adding-values-in-the-created-class
  :END:
...
#+NAME: all
#+BEGIN_SRC bash :results none :exports none :tangle ../../tests/use_cases/dynamic_parameters_advanced_use_cases.sh :noweb yes :shebang "#!/usr/bin/env bash"
  set -eu
<<init>>
