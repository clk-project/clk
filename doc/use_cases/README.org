...
* parameters, alias etc
  ...
  If you want to organize all of clk's commands under a custom namespace by
  creating an alias to the root command, see [[file:alias_to_root.org][alias to root]].
...
* batteries included
  ...
  When a command feels slow and you want to find the bottleneck, the
  [[file:spotting_slow_code.org][spotting slow code]] use case shows how to use ~--develop --timestamp~ to
...
* real world example: backing up documents
  ...
  starting from a Python group that shares a ~BACKUP_DESTINATION~ environment
  variable with all its bash subcommands. It demonstrates shared environment
  variables, hierarchical commands, auto-created groups, persisted parameters,
  flow dependencies, aliases, per-project configuration, and hostname extensions
