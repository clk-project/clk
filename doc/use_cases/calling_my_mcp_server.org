:PROPERTIES:
:ID:       0b6acd63-d091-4c6b-83f6-7d2dd3ce908f
:END:
#+language: en
#+EXPORT_FILE_NAME: ./calling_my_mcp_server.md
#+SUBTITLE: Getting a Cognito token for my MCP server on AgentCore, with its password kept out of files and parameters (clk secret, keyring, netrc, --ask-secret)

#+CALL: ../../lp.org:check-result()

#+NAME: init
#+BEGIN_SRC bash :results none :exports none :session 0b6acd63-d091-4c6b-83f6-7d2dd3ce908f
  . ./sandboxing.sh
#+END_SRC

I have an MCP server running on Amazon Bedrock AgentCore. It sits behind a
Cognito user pool, so before calling one of its tools I need an access token,
and to get one I log in as a test user, demo-buyer, with its password.

The token lasts an hour. Every hour, I read the password from a plain file with
~$(cat ...)~, hand it to ~aws cognito-idp initiate-auth~ and paste the token
where it goes. Let's make a command out of it, and get that password out of the
file on the way.

* a command to get my token
  :PROPERTIES:
  :CUSTOM_ID: a-command-to-get-my-token
  :END:

  Writing the password on the command line would leave it in my shell
  history, so the command prompts for it, hiding what I type and asking twice,
  the [[https://click.palletsprojects.com/en/8.1.x/options/#password-prompts][click way]]. In real life, ~token~ calls ~aws cognito-idp initiate-auth~.
  Here, it only says how it logs in.

  #+NAME: command
  #+BEGIN_SRC python :results none :exports code
    from clk.config import config
    from clk.decorators import group, option


    class AgentcoreConfig:
        pass


    @group()
    @option("--user", default="demo-buyer", help="The Cognito user to log in as", expose_class=AgentcoreConfig)
    @option(
        "--password",
        help="The password of that user",
        prompt=True,
        confirmation_prompt=True,
        hide_input=True,
        expose_class=AgentcoreConfig,
    )
    def agentcore():
        "Talk to my MCP server on Bedrock AgentCore"


    @agentcore.command()
    def token():
        "Get a fresh access token from Cognito"
        print(f"Logging in as {config.agentcore.user} with password {config.agentcore.password}")
  #+END_SRC

  #+NAME: createthecommand
  #+BEGIN_SRC bash :results none :exports none :session 0b6acd63-d091-4c6b-83f6-7d2dd3ce908f :noweb yes
    cat > "${TMP}/agentcore.py" <<'EOF'
    <<command>>
    EOF
    clk command create python agentcore --group --force --from-file "${TMP}/agentcore.py"
  #+END_SRC

  To know more about ~expose_class=AgentcoreConfig~, see [[file:dynamic_parameters_and_exposed_class.org][dynamic parameters]].

* not typing the password every hour
  :PROPERTIES:
  :CUSTOM_ID: not-typing-the-password-every-hour
  :END:

  Being prompted every hour gets old fast. I could make the password a
  parameter.

  #+NAME: setsecretinparameter
  #+BEGIN_SRC bash :results verbatim :exports both :session 0b6acd63-d091-4c6b-83f6-7d2dd3ce908f :cache yes
    clk parameter set agentcore --password mytoken
  #+END_SRC

  #+RESULTS[]: setsecretinparameter
  : New [36mglobal[0m parameters for agentcore: --password mytoken

  #+NAME: usesecretinparameter
  #+BEGIN_SRC bash :results verbatim :exports both :session 0b6acd63-d091-4c6b-83f6-7d2dd3ce908f :cache yes
    clk agentcore token
  #+END_SRC

  #+RESULTS[]: usesecretinparameter
  : Logging in as demo-buyer with password mytoken

  It works, but the password now sits in plain text in my clk parameters, no
  better than the file I started from.

  Instead, the parameter can tell clk to look the password up in my password
  manager.

  #+NAME: usethebearefromsecret
  #+BEGIN_SRC bash :results verbatim :exports both :session 0b6acd63-d091-4c6b-83f6-7d2dd3ce908f :cache yes
    clk parameter set agentcore --password noeval:secret:demo-buyer-password
  #+END_SRC

  #+RESULTS[]: usethebearefromsecret
  : Removing [36mglobal[0m parameters of agentcore: --password mytoken
  : New [36mglobal[0m parameters for agentcore: --password secret:demo-buyer-password

  1. ~secret:demo-buyer-password~ tells clk to get the value from my password
     manager, under the key ~demo-buyer-password~,
  2. ~noeval:~ keeps clk from looking it up right now, so that the parameter
     holds the reference rather than the password.

  I have not stored it yet, so clk fails and says so[fn:1].

  #+NAME: httpwithsecretfail
  #+BEGIN_SRC bash :results verbatim :exports both :session 0b6acd63-d091-4c6b-83f6-7d2dd3ce908f :cache yes
    clk agentcore token
  #+END_SRC

  #+RESULTS[]: httpwithsecretfail
  : [31merror: [0mCould not find the secret for demo-buyer-password

* on a machine where I have not stored it
  :PROPERTIES:
  :CUSTOM_ID: on-a-machine-where-i-have-not-stored-it
  :END:

  On a colleague's laptop, or on mine before I store it, ~--ask-secret~ makes
  clk ask me for the password rather than fail.

  #+NAME: askforpassword
  #+BEGIN_SRC bash :results none :exports code
    clk --ask-secret agentcore token
  #+END_SRC

  #+NAME: generate_expect_file_success
  #+BEGIN_SRC bash :results none :exports none :noweb yes :session 0b6acd63-d091-4c6b-83f6-7d2dd3ce908f
    cat<<"EOEXPECT" > "pass.exp"
    #!/usr/bin/env -S expect -f
    set timeout 30
    spawn <<askforpassword>>
    expect_after timeout { puts "timed out"; exit 1 }
    match_max 100000
    expect -exact "Could not find the secret for demo-buyer-password\r
    Please provide the secret demo-buyer-password: "
    send -- "test\r"
    expect -exact "\r
    Repeat for confirmation: "
    send -- "test\r"
    expect eof
    EOEXPECT
  #+END_SRC

  #+NAME: call_ask_for_real
  #+BEGIN_SRC bash :results verbatim :exports results :session 0b6acd63-d091-4c6b-83f6-7d2dd3ce908f :cache yes
    expect pass.exp |tail -n+2
  #+END_SRC

  #+RESULTS[]: call_ask_for_real
  : [33mwarning: [0mCould not find the secret for demo-buyer-password
  : Please provide the secret demo-buyer-password:
  : Repeat for confirmation:
  : Logging in as demo-buyer with password test

  The error became a warning, and I typed the password.

  #+NAME: generate_expect_file_error
  #+BEGIN_SRC bash :results none :exports none :noweb yes :session 0b6acd63-d091-4c6b-83f6-7d2dd3ce908f
    cat<<"EOEXPECT" > "passerror.exp"
    #!/usr/bin/env -S expect -f
    set timeout 30
    spawn <<askforpassword>>
    expect_after timeout { puts "timed out"; exit 1 }
    match_max 100000
    expect -exact "Could not find the secret for demo-buyer-password\r
    Please provide the secret demo-buyer-password: "
    send -- "something\r"
    expect -exact "\r
    Repeat for confirmation: "
    send -- "somethingelse\r"
    expect -exact "Error: The two entered values do not match.\r"
    expect -exact "Please provide the secret demo-buyer-password:"
    EOEXPECT
  #+END_SRC

  #+NAME: call_ask_for_real_error
  #+BEGIN_SRC bash :results verbatim :exports results :session 0b6acd63-d091-4c6b-83f6-7d2dd3ce908f :cache yes
    expect passerror.exp |tail -n+2
  #+END_SRC

  When I mistype it, clk asks again until both match.

  #+RESULTS[]: call_ask_for_real_error
  : [33mwarning: [0mCould not find the secret for demo-buyer-password
  : Please provide the secret demo-buyer-password:
  : Repeat for confirmation:
  : Error: The two entered values do not match.
  : Please provide the secret demo-buyer-password:

  The missing password does not get in the way of completing the command,
  with or without ~--ask-secret~.

  #+NAME: sequence-to-complete
  #+BEGIN_SRC bash :results none :exports none
    agentcore --us
  #+END_SRC

  #+NAME: ask-for-completion
  #+BEGIN_SRC bash :results none :exports code :noweb yes
    clk <<sequence-to-complete>><TAB>
  #+END_SRC

  #+NAME: try-completion
  #+BEGIN_SRC bash :results verbatim :exports results :session 0b6acd63-d091-4c6b-83f6-7d2dd3ce908f :noweb yes :cache yes
    clk completion try --remove-bash-formatting --last <<sequence-to-complete>>
  #+END_SRC

  #+RESULTS[]: try-completion
  : --user

  #+NAME: try-completion-without-ask-secret
  #+BEGIN_SRC bash :results verbatim :exports none :session 0b6acd63-d091-4c6b-83f6-7d2dd3ce908f :noweb yes :cache yes
    clk --ask-secret completion try --remove-bash-formatting --last <<sequence-to-complete>>
  #+END_SRC

  #+RESULTS[]: try-completion-without-ask-secret
  : --user

* storing it in my password manager
  :PROPERTIES:
  :CUSTOM_ID: storing-it-in-my-password-manager
  :END:

  clk stores secrets with [[https://github.com/jaraco/keyring][keyring]], hence in whatever password manager keyring
  is set up to use.

  #+NAME: providepassword
  #+BEGIN_SRC bash :results none :exports none :session 0b6acd63-d091-4c6b-83f6-7d2dd3ce908f
    clk secret set --set-parameter global --secret mytoken
  #+END_SRC

  #+NAME: storeasecret
  #+BEGIN_SRC bash :results none :exports code :session 0b6acd63-d091-4c6b-83f6-7d2dd3ce908f
    clk secret set demo-buyer-password
  #+END_SRC

  It prompts for the password, to keep it out of my shell history too. Let's
  say I typed "mytoken".

  #+NAME: showsecret
  #+BEGIN_SRC bash :results verbatim :exports both :session 0b6acd63-d091-4c6b-83f6-7d2dd3ce908f :cache yes
    clk secret show demo-buyer-password
  #+END_SRC

  #+RESULTS[]: showsecret
  : demo-buyer-password [2m*****[0m

  That tells me it is there without putting it on my screen. When I do want to
  see it, I say so.

  #+NAME: reallyshowsecret
  #+BEGIN_SRC bash :results verbatim :exports both :session 0b6acd63-d091-4c6b-83f6-7d2dd3ce908f :cache yes
    clk secret show demo-buyer-password --secret
  #+END_SRC

  #+RESULTS[]: reallyshowsecret
  : demo-buyer-password [2mmytoken[0m

  And to hand it to another program, the value alone.

  #+NAME: reallyshowonlysecret
  #+BEGIN_SRC bash :results verbatim :exports both :session 0b6acd63-d091-4c6b-83f6-7d2dd3ce908f :cache yes
    clk secret show demo-buyer-password --secret --field secret
  #+END_SRC

  #+RESULTS[]: reallyshowonlysecret
  : mytoken

  Now my command gets its password on its own, and I can delete that plain
  file.

  #+NAME: httpwithsecret
  #+BEGIN_SRC bash :results verbatim :exports both :session 0b6acd63-d091-4c6b-83f6-7d2dd3ce908f :cache yes
    clk agentcore token
  #+END_SRC

  #+RESULTS[]: httpwithsecret
  : Logging in as demo-buyer with password mytoken

  Calling a tool of the server is a bash command, whose password defaults to
  the secret.

  #+NAME: bashcall
  #+BEGIN_SRC bash :results none :exports code :session 0b6acd63-d091-4c6b-83f6-7d2dd3ce908f
    clk command create bash mcp-call --description "Call a tool of my MCP server" \
        --option '--password:str:The password of demo-buyer:{"default": "secret:demo-buyer-password"}' \
        --argument 'tool:str:The tool to call' \
        --body 'echo "Calling ${CLK___TOOL} as demo-buyer with password ${CLK___PASSWORD}"'
  #+END_SRC

  #+NAME: runbashcall
  #+BEGIN_SRC bash :results verbatim :exports both :session 0b6acd63-d091-4c6b-83f6-7d2dd3ce908f :cache yes
    clk mcp-call get_account_info
  #+END_SRC

  #+RESULTS[]: runbashcall
  : Calling get_account_info as demo-buyer with password mytoken

* setting up a new laptop
  :PROPERTIES:
  :CUSTOM_ID: setting-up-a-new-laptop
  :END:

  My clk parameters and aliases follow me from laptop to laptop, my password
  manager does not always. One of those aliases gets the token of demo-buyer
  whatever the parameters say.

  #+NAME: secretalias
  #+BEGIN_SRC bash :results none :exports code :session 0b6acd63-d091-4c6b-83f6-7d2dd3ce908f
    clk alias set buyer-token agentcore --user demo-buyer --password noeval:secret:demo-buyer-password token
  #+END_SRC

  To know which secrets to store again, clk lists the ones they refer to, and
  whether each is there.

  #+NAME: listsecrets
  #+BEGIN_SRC bash :results verbatim :exports both :session 0b6acd63-d091-4c6b-83f6-7d2dd3ce908f :cache yes
    clk secret list
  #+END_SRC

  #+RESULTS[]: listsecrets
  : key                  status    commands
  : -------------------  --------  ---------------------
  : demo-buyer-password  [32mset[0m       [36m[2magentcore[0m [36m[2mbuyer-token[0m
  : [2m--------------[0m
  : Legend: [36m[2mglobal[0m

* a check of my own in python
  :PROPERTIES:
  :CUSTOM_ID: a-check-of-my-own-in-python
  :END:

  I also wrote a quick python command that logs in with its own http client.
  It reads the same password with ~get_secret~.

  #+NAME: getsecretfromcode
  #+BEGIN_SRC python :results none :exports code
    from clk import get_secret

    @command()
    def dosomething():
        'Log in with my own client'
        click.echo(get_secret('demo-buyer-password'))
  #+END_SRC

  #+NAME: testgetsecret
  #+BEGIN_SRC bash :results none :exports none :session 0b6acd63-d091-4c6b-83f6-7d2dd3ce908f :noweb yes
    clk command create python dosomething --force
    cat<<EOF >> "${CLKCONFIGDIR}/python/dosomething.py"
    <<getsecretfromcode>>
    EOF
  #+END_SRC

  #+NAME: showgetsecret
  #+BEGIN_SRC bash :results verbatim :exports both :session 0b6acd63-d091-4c6b-83f6-7d2dd3ce908f :cache yes
    clk dosomething
  #+END_SRC

  #+RESULTS[]: showgetsecret
  : mytoken

* when the demo-buyer password is reset
  :PROPERTIES:
  :CUSTOM_ID: when-the-demo-buyer-password-is-reset
  :END:

  When someone resets the demo-buyer password, I remove the old one.

  #+NAME: unsetshown
  #+BEGIN_SRC bash :results none :exports code
    clk secret unset demo-buyer-password
  #+END_SRC

  clk asks first. I hesitate, since the reset may not be done yet, and answer
  no.

  #+NAME: refusingtheremoval
  #+BEGIN_SRC bash :results verbatim :exports results :session 0b6acd63-d091-4c6b-83f6-7d2dd3ce908f :cache yes
    clk secret unset demo-buyer-password <<< n
  #+END_SRC

  #+RESULTS[]: refusingtheremoval
  : This will definitely remove the secret for demo-buyer-password. Are you sure? [y/N]: Kept the secret for demo-buyer-password

  It is still there.

  #+NAME: stillthere
  #+BEGIN_SRC bash :results verbatim :exports both :session 0b6acd63-d091-4c6b-83f6-7d2dd3ce908f :cache yes
    clk secret show demo-buyer-password
  #+END_SRC

  #+RESULTS[]: stillthere
  : demo-buyer-password [2m*****[0m

  #+NAME: forceunset
  #+BEGIN_SRC bash :results none :exports none :session 0b6acd63-d091-4c6b-83f6-7d2dd3ce908f
    clk parameter set secret.unset --force
  #+END_SRC

  The reset is done, so this time I answer yes. In a script, ~--force~ skips
  the question.

  #+NAME: removingsecret
  #+BEGIN_SRC bash :results none :exports code :session 0b6acd63-d091-4c6b-83f6-7d2dd3ce908f
    clk secret unset demo-buyer-password
  #+END_SRC

  Asking for it now fails, so a script that reads it notices.

  #+NAME: checkthatthesecretisgone
  #+BEGIN_SRC bash :results verbatim :exports both :session 0b6acd63-d091-4c6b-83f6-7d2dd3ce908f :cache yes
    clk secret show demo-buyer-password
  #+END_SRC

  #+RESULTS[]: checkthatthesecretisgone
  : error: No secret set

  Removing it again tells me there is nothing left to remove.

  #+NAME: unsetmissingsecret
  #+BEGIN_SRC bash :results verbatim :exports both :session 0b6acd63-d091-4c6b-83f6-7d2dd3ce908f :cache yes
    clk secret unset demo-buyer-password
  #+END_SRC

  #+RESULTS[]: unsetmissingsecret
  : error: No secret set

  And the list reminds me to store the new one.

  #+NAME: listmissingsecrets
  #+BEGIN_SRC bash :results verbatim :exports both :session 0b6acd63-d091-4c6b-83f6-7d2dd3ce908f :cache yes
    clk secret list
  #+END_SRC

  #+RESULTS[]: listmissingsecrets
  : key                  status    commands
  : -------------------  --------  ---------------------
  : demo-buyer-password  [31mmissing[0m   [36m[2magentcore[0m [36m[2mbuyer-token[0m
  : [2m--------------[0m
  : Legend: [36m[2mglobal[0m

* when an agent runs it for me
  :PROPERTIES:
  :CUSTOM_ID: when-an-agent-runs-it-for-me
  :END:

  These days, a coding agent calls my MCP server as often as I do. It runs in a
  shell with no desktop session, where keyring has no backend to work with, and
  it cannot answer a prompt either. There, I keep the password in ~~/.netrc~:
  the secret name in ~machine~, its value in ~password~.

  #+NAME: netrc_content
  #+BEGIN_SRC authinfo :results none :exports code
    machine demo-buyer-password
    password thevalue
  #+END_SRC

  #+NAME: generate_netrc
  #+BEGIN_SRC bash :results none :exports none :session 0b6acd63-d091-4c6b-83f6-7d2dd3ce908f :noweb yes
    cat <<EOF > "${CLK_NETRC_LOCATION}"
    <<netrc_content>>
    EOF
  #+END_SRC

  Sometimes the agent runs in a container where clk was installed without the
  python library keyring. clk reads my netrc file all the same.

  #+NAME: hidekeyring
  #+BEGIN_SRC bash :results none :exports none :session 0b6acd63-d091-4c6b-83f6-7d2dd3ce908f
    mkdir -p "${TMP}/nokeyring/keyring"
    echo "raise ModuleNotFoundError(\"No module named 'keyring'\")" > "${TMP}/nokeyring/keyring/__init__.py"
    OLDPYTHONPATH="${PYTHONPATH-}"
    export PYTHONPATH="${TMP}/nokeyring${PYTHONPATH:+:${PYTHONPATH}}"
  #+END_SRC

  #+NAME: nokeyring_show
  #+BEGIN_SRC bash :results verbatim :exports both :session 0b6acd63-d091-4c6b-83f6-7d2dd3ce908f :cache yes
    clk secret show demo-buyer-password --secret
  #+END_SRC

  #+RESULTS[]: nokeyring_show
  : demo-buyer-password [2mthevalue[0m

  #+NAME: nokeyring_backend
  #+BEGIN_SRC bash :results verbatim :exports both :session 0b6acd63-d091-4c6b-83f6-7d2dd3ce908f :cache yes
    clk secret backend which
  #+END_SRC

  #+RESULTS[]: nokeyring_backend
  : clk keeps your secrets in clk.netrc.Netrc, because the python library keyring is not installed.

  #+NAME: nokeyring_backends
  #+BEGIN_SRC bash :results verbatim :exports both :session 0b6acd63-d091-4c6b-83f6-7d2dd3ce908f :cache yes
    clk secret backend show
  #+END_SRC

  #+RESULTS[]: nokeyring_backends
  : backend          configuration      priority  status
  : ---------------  ---------------  ----------  --------
  : clk.netrc.Netrc  Unset                     1  in use

  Naming a keyring there makes no sense, and clk says so.

  #+NAME: nokeyring_named
  #+BEGIN_SRC bash :results verbatim :exports both :session 0b6acd63-d091-4c6b-83f6-7d2dd3ce908f :cache yes
    clk --keyring clk.keyrings.NetrcKeyring secret show demo-buyer-password
  #+END_SRC

  #+RESULTS[]: nokeyring_named
  : Usage: clk [OPTIONS] [COMMAND] [ARGS]...
  : error: You need to install keyring in order to manipulate secrets. Hint: python3 -m pip install keyring

  #+NAME: bringbackkeyring
  #+BEGIN_SRC bash :results none :exports none :session 0b6acd63-d091-4c6b-83f6-7d2dd3ce908f
    export PYTHONPATH="${OLDPYTHONPATH}"
  #+END_SRC

  Where keyring is installed but finds no password manager, clk falls back on
  netrc by itself.

  #+NAME: nobackend
  #+BEGIN_SRC bash :results none :exports none :session 0b6acd63-d091-4c6b-83f6-7d2dd3ce908f
    clk parameter set clk --keyring keyring.backends.fail.Keyring
  #+END_SRC

  #+NAME: netrc_fallback
  #+BEGIN_SRC bash :results verbatim :exports both :session 0b6acd63-d091-4c6b-83f6-7d2dd3ce908f :cache yes
    clk secret show demo-buyer-password --secret
  #+END_SRC

  #+RESULTS[]: netrc_fallback
  : demo-buyer-password [2mthevalue[0m

  When I wonder where a secret comes from, clk tells me where it keeps them,
  and why.

  #+NAME: nodummy
  #+BEGIN_SRC bash :results none :exports none :session 0b6acd63-d091-4c6b-83f6-7d2dd3ce908f
    unset DUMMYFILEKEYRINGPATH
  #+END_SRC

  #+NAME: whichbackend
  #+BEGIN_SRC bash :results verbatim :exports both :session 0b6acd63-d091-4c6b-83f6-7d2dd3ce908f :cache yes
    clk secret backend which
  #+END_SRC

  #+RESULTS[]: whichbackend
  : clk keeps your secrets in clk.keyrings.NetrcKeyring, because the python library keyring found no password manager on this machine.

  And which keyrings it could use instead.

  #+NAME: showbackends
  #+BEGIN_SRC bash :results verbatim :exports both :session 0b6acd63-d091-4c6b-83f6-7d2dd3ce908f :cache yes
    clk secret backend show
  #+END_SRC

  #+RESULTS[]: showbackends
  : backend                                  configuration      priority  status
  : ---------------------------------------  ---------------  ----------  --------
  : keyring.backends.chainer.ChainerBackend  Unset                    10
  : clk.keyrings.GpgKeyring                  Unset                     1
  : clk.keyrings.NetrcKeyring                Unset                     1  in use
  : keyring.backends.fail.Keyring            Unset                     0

  I can also ask for netrc explicitly, with the ~NetrcKeyring~ of
  ~clk.keyrings~.

  #+NAME: using_netrc
  #+BEGIN_SRC bash :results verbatim :exports both :session 0b6acd63-d091-4c6b-83f6-7d2dd3ce908f :cache yes
    clk --keyring clk.keyrings.NetrcKeyring secret show demo-buyer-password --secret
  #+END_SRC

  #+RESULTS[]: using_netrc
  : demo-buyer-password [2mthevalue[0m

  A secret my netrc file does not hold is simply not there.

  #+NAME: netrc_missing
  #+BEGIN_SRC bash :results verbatim :exports both :session 0b6acd63-d091-4c6b-83f6-7d2dd3ce908f :cache yes
    clk --keyring clk.keyrings.NetrcKeyring secret show other_token --secret
  #+END_SRC

  #+RESULTS[]: netrc_missing
  : error: No secret set

  It only reads: storing one is up to me, in the file itself.

  #+NAME: netrc_readonly
  #+BEGIN_SRC bash :results verbatim :exports both :session 0b6acd63-d091-4c6b-83f6-7d2dd3ce908f :cache yes
    clk --keyring clk.keyrings.NetrcKeyring secret set other_token
  #+END_SRC

  #+RESULTS[]: netrc_readonly
  : [31merror: [0mCould not save your secret.
  : Usage: clk secret set [OPTIONS] KEY
  : error: The netrc keyring only reads secrets. Write this one in your netrc file to use it.

  Removing one is up to me as well.

  #+NAME: netrc_readonly_unset
  #+BEGIN_SRC bash :results verbatim :exports both :session 0b6acd63-d091-4c6b-83f6-7d2dd3ce908f :cache yes
    clk --keyring clk.keyrings.NetrcKeyring secret unset demo-buyer-password
  #+END_SRC

  #+RESULTS[]: netrc_readonly_unset
  : Usage: clk secret unset [OPTIONS] KEY
  : error: The netrc keyring only reads secrets. Remove this one from your netrc file to get rid of it.

* when the team shares the password
  :PROPERTIES:
  :CUSTOM_ID: when-the-team-shares-the-password
  :END:

  The team now keeps the demo-buyer password in AWS Secrets Manager, and my
  role may read it, not write it. I wrote a keyring backend that reads from
  there. In real life, ~get_password~ calls ~aws secretsmanager
  get-secret-value~.

  #+NAME: readonly_keyring
  #+BEGIN_SRC python :results none :exports code
    import keyring.backend


    class SecretsManagerKeyring(keyring.backend.KeyringBackend):
        priority = 6

        def get_password(self, service, username):
            return None

        def set_password(self, service, username, password):
            raise NotImplementedError

        def delete_password(self, service, username):
            raise NotImplementedError
  #+END_SRC

  #+NAME: install_readonly_keyring
  #+BEGIN_SRC bash :results none :exports none :session 0b6acd63-d091-4c6b-83f6-7d2dd3ce908f :noweb yes
    mkdir -p "${TMP}/pylib"
    cat <<'EOF' > "${TMP}/pylib/team_keyring.py"
    <<readonly_keyring>>
    EOF
    export PYTHONPATH="${TMP}/pylib${PYTHONPATH:+:${PYTHONPATH}}"
  #+END_SRC

  Out of habit, I tried storing the new password after it was reset. clk says
  which keyring refused.

  #+NAME: readonly_keyring_set
  #+BEGIN_SRC bash :results verbatim :exports both :session 0b6acd63-d091-4c6b-83f6-7d2dd3ce908f :cache yes
    clk --keyring team_keyring.SecretsManagerKeyring secret set demo-buyer-password
  #+END_SRC

  #+RESULTS[]: readonly_keyring_set
  : error: The keyring team_keyring.SecretsManagerKeyring cannot store secrets. Store it with the tool of that password manager, or pick another keyring with --keyring.

  #+NAME: teambackend
  #+BEGIN_SRC bash :results verbatim :exports both :session 0b6acd63-d091-4c6b-83f6-7d2dd3ce908f :cache yes
    clk --keyring team_keyring.SecretsManagerKeyring secret backend which
  #+END_SRC

  #+RESULTS[]: teambackend
  : clk keeps your secrets in team_keyring.SecretsManagerKeyring, because --keyring names it.

  #+NAME: backendfound
  #+BEGIN_SRC bash :results none :exports none :session 0b6acd63-d091-4c6b-83f6-7d2dd3ce908f
    clk parameter unset clk
  #+END_SRC

  Rather than typing ~--keyring~ every time, I tell clk to use it from now on.

  #+NAME: useteambackend
  #+BEGIN_SRC bash :results verbatim :exports both :session 0b6acd63-d091-4c6b-83f6-7d2dd3ce908f :cache yes
    clk secret backend use team_keyring.SecretsManagerKeyring
  #+END_SRC

  #+RESULTS[]: useteambackend
  : clk now keeps your secrets in team_keyring.SecretsManagerKeyring ([36mglobal[0m settings)

  #+NAME: showteambackend
  #+BEGIN_SRC bash :results verbatim :exports both :session 0b6acd63-d091-4c6b-83f6-7d2dd3ce908f :cache yes
    clk secret backend which
  #+END_SRC

  #+RESULTS[]: showteambackend
  : clk keeps your secrets in team_keyring.SecretsManagerKeyring, because the [36mglobal[0m settings name it.

  #+NAME: showteambackends
  #+BEGIN_SRC bash :results verbatim :exports both :session 0b6acd63-d091-4c6b-83f6-7d2dd3ce908f :cache yes
    clk secret backend show
  #+END_SRC

  #+RESULTS[]: showteambackends
  : backend                                  configuration      priority  status
  : ---------------------------------------  ---------------  ----------  --------
  : team_keyring.SecretsManagerKeyring       [36m[2mglobal[0m                    6  in use
  : keyring.backends.fail.Keyring            Unset                     0
  : keyring.backends.chainer.ChainerBackend  Unset                    -1
  : [2m--------------[0m
  : Legend: [36m[2mglobal[0m

* keeping it in the project
  :PROPERTIES:
  :CUSTOM_ID: keeping-it-in-the-project
  :END:

  The demo-buyer password is a throwaway, not worth a place in a password
  manager. I keep it in the project of my MCP server instead, encrypted with my
  gpg key.

  #+NAME: gpgsetup
  #+BEGIN_SRC bash :results none :exports none :session 0b6acd63-d091-4c6b-83f6-7d2dd3ce908f
    export GNUPGHOME="${TMP}/gnupg"
    mkdir -m 700 "${GNUPGHOME}"
    gpg --batch --quiet --passphrase '' --quick-gen-key me@example.com default default never 2>/dev/null
    mkdir -p "${TMP}/mcp-server/.clk"
    cd "${TMP}/mcp-server"
  #+END_SRC

  I give it a try first.

  #+NAME: gpgnoid
  #+BEGIN_SRC bash :results verbatim :exports both :session 0b6acd63-d091-4c6b-83f6-7d2dd3ce908f :cache yes
    clk --keyring clk.keyrings.GpgKeyring secret set demo-buyer-password
  #+END_SRC

  #+RESULTS[]: gpgnoid
  : [31merror: [0mCould not save your secret.
  : Usage: clk secret set [OPTIONS] KEY
  : error: Write in ./.clk/secrets/.gpg-id the gpg ids to encrypt the secrets to, one per line. gpg --list-secret-keys shows yours.

  #+NAME: gpgid
  #+BEGIN_SRC bash :results none :exports code :session 0b6acd63-d091-4c6b-83f6-7d2dd3ce908f
    mkdir -p .clk/secrets
    echo me@example.com > .clk/secrets/.gpg-id
  #+END_SRC

  Then I make it the keyring of the project. I don't remember its name, tab
  does.

  #+NAME: completegpg-shown
  #+BEGIN_SRC bash :results none :exports code
    clk secret backend --local use clk.keyrings.<TAB>
  #+END_SRC

  #+NAME: completegpg
  #+BEGIN_SRC bash :results verbatim :exports results :session 0b6acd63-d091-4c6b-83f6-7d2dd3ce908f :cache yes
    clk completion try --remove-bash-formatting --last secret backend --local use clk.keyrings.
  #+END_SRC

  #+RESULTS[]: completegpg
  : clk.keyrings.GpgKeyring
  : clk.keyrings.NetrcKeyring

  #+NAME: usegpg
  #+BEGIN_SRC bash :results verbatim :exports both :session 0b6acd63-d091-4c6b-83f6-7d2dd3ce908f :cache yes
    clk secret backend --local use clk.keyrings.GpgKeyring
  #+END_SRC

  #+RESULTS[]: usegpg
  : clk now keeps your secrets in clk.keyrings.GpgKeyring ([32mlocal[0m settings)

  #+NAME: gpgset
  #+BEGIN_SRC bash :results none :exports code :session 0b6acd63-d091-4c6b-83f6-7d2dd3ce908f
    clk secret set demo-buyer-password
  #+END_SRC

  #+NAME: gpgfiles
  #+BEGIN_SRC bash :results verbatim :exports both :session 0b6acd63-d091-4c6b-83f6-7d2dd3ce908f :cache yes
    ls .clk/secrets
  #+END_SRC

  #+RESULTS[]: gpgfiles
  : demo-buyer-password.gpg

  #+NAME: gpgshow
  #+BEGIN_SRC bash :results verbatim :exports both :session 0b6acd63-d091-4c6b-83f6-7d2dd3ce908f :cache yes
    clk secret show demo-buyer-password --secret
  #+END_SRC

  #+RESULTS[]: gpgshow
  : demo-buyer-password [2mmytoken[0m

  Removing it removes the file.

  #+NAME: gpgunset
  #+BEGIN_SRC bash :results verbatim :exports both :session 0b6acd63-d091-4c6b-83f6-7d2dd3ce908f :cache yes
    clk secret unset demo-buyer-password
    ls .clk/secrets
    clk secret show demo-buyer-password
  #+END_SRC

  #+RESULTS[]: gpgunset
  : error: No secret set

  With no secret left in it, the project goes back to the keyring of my
  laptop.

  #+NAME: unusegpg
  #+BEGIN_SRC bash :results verbatim :exports both :session 0b6acd63-d091-4c6b-83f6-7d2dd3ce908f :cache yes
    clk secret backend --local unuse
  #+END_SRC

  #+RESULTS[]: unusegpg
  : clk no longer picks a keyring

  On my desktop, the password manager of my session, behind the Secret Service
  interface, does just as well, and I don't need to handle gpg ids.

  #+NAME: secretservicesetup
  #+BEGIN_SRC bash :results none :exports none :session 0b6acd63-d091-4c6b-83f6-7d2dd3ce908f
    export DBUS_SESSION_BUS_ADDRESS="$(dbus-daemon --session --fork --print-address)"
    echo -n desktop | gnome-keyring-daemon --unlock --components=secrets > /dev/null
  #+END_SRC

  #+NAME: usesecretservice
  #+BEGIN_SRC bash :results verbatim :exports both :session 0b6acd63-d091-4c6b-83f6-7d2dd3ce908f :cache yes
    clk secret backend --local use keyring.backends.SecretService.Keyring
  #+END_SRC

  #+RESULTS[]: usesecretservice
  : clk now keeps your secrets in keyring.backends.SecretService.Keyring ([32mlocal[0m settings)

  #+NAME: secretserviceset
  #+BEGIN_SRC bash :results none :exports code :session 0b6acd63-d091-4c6b-83f6-7d2dd3ce908f
    clk secret set demo-buyer-password
  #+END_SRC

  #+NAME: secretserviceshow
  #+BEGIN_SRC bash :results verbatim :exports both :session 0b6acd63-d091-4c6b-83f6-7d2dd3ce908f :cache yes
    clk secret show demo-buyer-password --secret
    clk secret backend which
  #+END_SRC

  #+RESULTS[]: secretserviceshow
  : demo-buyer-password [2mmytoken[0m
  : clk keeps your secrets in keyring.backends.SecretService.Keyring, because the [32mlocal[0m settings name it.

  #+NAME: unusesecretservice
  #+BEGIN_SRC bash :results none :exports none :session 0b6acd63-d091-4c6b-83f6-7d2dd3ce908f
    clk secret unset demo-buyer-password
    clk secret backend --local unuse
  #+END_SRC

  #+NAME: leaveproject
  #+BEGIN_SRC bash :results none :exports none :session 0b6acd63-d091-4c6b-83f6-7d2dd3ce908f
    cd "${TMP}"
  #+END_SRC

* when the demo is over
  :PROPERTIES:
  :CUSTOM_ID: when-the-demo-is-over
  :END:

  The demo-buyer user is gone, and so are its parameter and its alias.

  #+NAME: dropparameter
  #+BEGIN_SRC bash :results none :exports code :session 0b6acd63-d091-4c6b-83f6-7d2dd3ce908f
    clk parameter unset agentcore
    clk alias unset buyer-token
  #+END_SRC

  I no longer need the team keyring either.

  #+NAME: unuseteambackend
  #+BEGIN_SRC bash :results verbatim :exports both :session 0b6acd63-d091-4c6b-83f6-7d2dd3ce908f :cache yes
    clk secret backend unuse
  #+END_SRC

  #+RESULTS[]: unuseteambackend
  : clk no longer picks a keyring

  Once is enough.

  #+NAME: unuseagain
  #+BEGIN_SRC bash :results verbatim :exports both :session 0b6acd63-d091-4c6b-83f6-7d2dd3ce908f :cache yes
    clk secret backend unuse
  #+END_SRC

  #+RESULTS[]: unuseagain
  : error: The global settings pick no keyring

  Without a choice of clk, the python library keyring picks the backend. It
  first reads the environment variable PYTHON_KEYRING_BACKEND.

  #+NAME: envbackend
  #+BEGIN_SRC bash :results verbatim :exports both :session 0b6acd63-d091-4c6b-83f6-7d2dd3ce908f :cache yes
    PYTHON_KEYRING_BACKEND=clk.keyrings.NetrcKeyring clk secret backend which
  #+END_SRC

  #+RESULTS[]: envbackend
  : clk keeps your secrets in clk.keyrings.NetrcKeyring, because the environment variable PYTHON_KEYRING_BACKEND names it.

  #+NAME: noenv
  #+BEGIN_SRC bash :results none :exports none :session 0b6acd63-d091-4c6b-83f6-7d2dd3ce908f
    unset PYTHON_KEYRING_BACKEND
  #+END_SRC

  Then its configuration file.

  #+NAME: keyringrc
  #+BEGIN_SRC bash :results none :exports code :session 0b6acd63-d091-4c6b-83f6-7d2dd3ce908f
    mkdir -p "${XDG_CONFIG_HOME}/python_keyring"
    cat <<EOF > "${XDG_CONFIG_HOME}/python_keyring/keyringrc.cfg"
    [backend]
    default-keyring=clk.keyrings.NetrcKeyring
    EOF
  #+END_SRC

  #+NAME: rcbackend
  #+BEGIN_SRC bash :results verbatim :exports both :session 0b6acd63-d091-4c6b-83f6-7d2dd3ce908f :cache yes
    clk secret backend which
  #+END_SRC

  #+RESULTS[]: rcbackend
  : clk keeps your secrets in clk.keyrings.NetrcKeyring, because ./config/python_keyring/keyringrc.cfg names it.

  Without either, it takes the backend of highest priority, here the Secret
  Service of my session.

  #+NAME: prioritybackend
  #+BEGIN_SRC bash :results verbatim :exports both :session 0b6acd63-d091-4c6b-83f6-7d2dd3ce908f :cache yes
    rm "${XDG_CONFIG_HOME}/python_keyring/keyringrc.cfg"
    clk secret backend which
  #+END_SRC

  #+RESULTS[]: prioritybackend
  : clk keeps your secrets in keyring.backends.SecretService.Keyring, because it has the highest priority of the backends the python library keyring found.

  Once the team publishes its Secrets Manager keyring as a package, installing
  it is enough: keyring finds it beside the Secret Service, and chains the two,
  asking the team keyring first.

  #+NAME: installteamkeyring-shown
  #+BEGIN_SRC bash :results none :exports code
    pip install team-keyring
  #+END_SRC

  #+NAME: installteamkeyring
  #+BEGIN_SRC bash :results none :exports none :session 0b6acd63-d091-4c6b-83f6-7d2dd3ce908f
    mkdir -p "${TMP}/pylib/team_keyring-1.0.dist-info"
    cat <<EOF > "${TMP}/pylib/team_keyring-1.0.dist-info/METADATA"
    Metadata-Version: 2.1
    Name: team-keyring
    Version: 1.0
    EOF
    cat <<EOF > "${TMP}/pylib/team_keyring-1.0.dist-info/entry_points.txt"
    [keyring.backends]
    team = team_keyring
    EOF
  #+END_SRC

  #+NAME: packagedbackend
  #+BEGIN_SRC bash :results verbatim :exports both :session 0b6acd63-d091-4c6b-83f6-7d2dd3ce908f :cache yes
    clk secret backend which
  #+END_SRC

  #+RESULTS[]: packagedbackend
  : clk keeps your secrets in keyring.backends.chainer.ChainerBackend, because it has the highest priority of the backends the python library keyring found.

  No command of mine needs a secret any more.

  #+NAME: listnosecret
  #+BEGIN_SRC bash :results verbatim :exports both :session 0b6acd63-d091-4c6b-83f6-7d2dd3ce908f :cache yes
    clk secret list
  #+END_SRC

  #+RESULTS[]: listnosecret
  : No parameter or alias refers to a secret

  #+NAME: test
  #+BEGIN_SRC bash :results none :exports none :noweb yes :shebang "#!/usr/bin/env bash" :tangle ../../tests/use_cases/calling_my_mcp_server.sh
    set -eu
    <<init>>

    <<createthecommand>>

    check-result(setsecretinparameter)

    check-result(usesecretinparameter)

    check-result(usethebearefromsecret)

    check-result(httpwithsecretfail)

    <<generate_expect_file_success>>

    check-result(call_ask_for_real)

    <<generate_expect_file_error>>

    check-result(call_ask_for_real_error)

    check-result(try-completion)

    check-result(try-completion-without-ask-secret)

    <<providepassword>>

    <<storeasecret>>

    check-result(showsecret)

    check-result(reallyshowsecret)

    check-result(reallyshowonlysecret)

    check-result(httpwithsecret)

    <<bashcall>>

    check-result(runbashcall)

    <<secretalias>>

    check-result(listsecrets)

    <<testgetsecret>>

    check-result(showgetsecret)

    check-result(refusingtheremoval)

    check-result(stillthere)

    <<forceunset>>

    <<removingsecret>>

    check-result(checkthatthesecretisgone)

    check-result(unsetmissingsecret)

    check-result(listmissingsecrets)

    <<generate_netrc>>

    <<hidekeyring>>

    check-result(nokeyring_show)

    check-result(nokeyring_backend)

    check-result(nokeyring_backends)

    check-result(nokeyring_named)

    <<bringbackkeyring>>

    <<nobackend>>

    check-result(netrc_fallback)

    <<nodummy>>

    check-result(whichbackend)

    check-result(showbackends)

    check-result(using_netrc)

    check-result(netrc_missing)

    check-result(netrc_readonly)

    check-result(netrc_readonly_unset)

    <<install_readonly_keyring>>

    check-result(readonly_keyring_set)

    check-result(teambackend)

    <<backendfound>>

    check-result(useteambackend)

    check-result(showteambackend)

    check-result(showteambackends)

    <<gpgsetup>>

    check-result(gpgnoid)

    <<gpgid>>

    check-result(completegpg)

    check-result(usegpg)

    <<gpgset>>

    check-result(gpgfiles)

    check-result(gpgshow)

    check-result(gpgunset)

    check-result(unusegpg)

    <<secretservicesetup>>

    check-result(usesecretservice)

    <<secretserviceset>>

    check-result(secretserviceshow)

    <<unusesecretservice>>

    <<leaveproject>>

    <<dropparameter>>

    check-result(unuseteambackend)


    check-result(unuseagain)

    check-result(envbackend)

    <<noenv>>

    <<keyringrc>>

    check-result(rcbackend)

    check-result(prioritybackend)

    <<installteamkeyring>>

    check-result(packagedbackend)

    check-result(listnosecret)

  #+END_SRC

* Footnotes
  :PROPERTIES:
  :CUSTOM_ID: footnotes
  :END:

[fn:1] so that a script calling clk learns that something went wrong
