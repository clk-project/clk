...
#+SUBTITLE: Built-in bash helpers: clk_drop_duplicate and other utilities in _clk.sh
...
Now that [[file:checking_my_server.org][you can write a bash command]]. you might want to do some complicated
...
** dropping duplicate lines
   :PROPERTIES:
   :CUSTOM_ID: dropping-duplicate-lines
   :END:
...
* tangle                                                           :noexport:
  ...
  :CUSTOM_ID: tangle
  :END:
  #+BEGIN_SRC bash :exports none :tangle ../../tests/use_cases/bash_command_built_in_lib.sh :noweb yes :shebang "#!/usr/bin/env bash"
  set -eu
    <<init>>
