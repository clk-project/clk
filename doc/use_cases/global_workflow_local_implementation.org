...
#+SUBTITLE: Defining workflows globally while letting each project supply its own implementation
...
* defining your workflow globally
  :PROPERTIES:
  :CUSTOM_ID: defining-your-workflow-globally
  :END:
...
* filling in the details for a specific project
  :PROPERTIES:
  :CUSTOM_ID: filling-in-the-details-for-a-specific-project
  :END:
...
* the workflow adapts to context
  :PROPERTIES:
  :CUSTOM_ID: the-workflow-adapts-to-context
  :END:
...
* going further: project-specific workflows
  :PROPERTIES:
  :CUSTOM_ID: going-further-project-specific-workflows
  :END:
...
#+BEGIN_SRC bash :exports none :tangle ../../tests/use_cases/global_workflow_local_implementation.sh :noweb yes :shebang "#!/usr/bin/env bash"
  set -eu
<<init>>
