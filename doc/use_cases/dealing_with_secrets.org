...
* using your own secret provider, or the built-in netrc one
  ...
  You need not name it, though. On a machine where keyring finds no backend to
  work with, clk falls back on netrc by itself.
  ...
  #+NAME: test
  #+BEGIN_SRC bash :results none :exports none :noweb yes :shebang "#!/usr/bin/env bash" :tangle ../../tests/use_cases/dealing_with_secrets.sh
    set -eu
    <<init>>
