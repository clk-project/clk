...
Run that line again, having forgotten you already did, and clk would rather say no
than write over the project you started.
...
#+RESULTS[]: csm-fork-again
: Usage: clk fork [OPTIONS] NAME
: error: csm already exist

When starting over is what you want, ~--force~ clears the way, and your first
...
* creating the simulator commands
  :PROPERTIES:
  :CUSTOM_ID: creating-the-simulator-commands
  :END:
...
Now we can ask for a debug build with the unity trick on, and measure the
coverage while we are at it. Note that we never taught the command what unity
means: we only wrote it down, and that is enough for the help, the completion and
the parameters to know about it. Coverage is another matter, that is our word for
it, and we translate it.

#+NAME: csm-configure-options
#+BEGIN_SRC bash :results verbatim :exports both :session 807c02cd-a6cf-432f-b982-530fc81dbbf4 :cache yes
csm configure --unity --build-type Debug --coverage
#+END_SRC

#+RESULTS[]: csm-configure-options
: Configuring build system with --unity --build-type Debug --analysis coverage

You can provide the option several times.

#+NAME: csm-configure-defines
#+BEGIN_SRC bash :results verbatim :exports both :session 807c02cd-a6cf-432f-b982-530fc81dbbf4 :cache yes
csm configure --define WITH_MPI=ON --define CHAOS_SEED=42
#+END_SRC

#+RESULTS[]: csm-configure-defines
: Configuring build system with --define WITH_MPI=ON --define CHAOS_SEED=42

#+NAME: csm-build-options
#+BEGIN_SRC bash :results verbatim :exports both :session 807c02cd-a6cf-432f-b982-530fc81dbbf4 :cache yes
csm build --jobs 8 --target simulator --target tests
#+END_SRC

#+RESULTS[]: csm-build-options
...
The build left a directory behind, and ~csm clean~ throws it away. A command
that says ~handle_dry_run=True~ can be asked what it would do rather than do it,
and the helpers of ~clk.lib~ hold back on their own.

#+NAME: csm-clean-dry-run
#+BEGIN_SRC bash :results verbatim :exports both :session 807c02cd-a6cf-432f-b982-530fc81dbbf4 :cache yes
csm --dry-run clean
test -d build && echo "build is still there"
#+END_SRC

#+RESULTS[]: csm-clean-dry-run
: (dry-run) remove build
: build is still there

Nothing was removed. Now for real.

#+NAME: csm-clean-for-real
#+BEGIN_SRC bash :results verbatim :exports both :session 807c02cd-a6cf-432f-b982-530fc81dbbf4 :cache yes
csm clean
test -d build || echo "build is gone"
#+END_SRC

#+RESULTS[]: csm-clean-for-real
: build is gone

It went quietly. With ~--action~ it says what it is doing as it goes.

#+NAME: csm-clean-aloud
#+BEGIN_SRC bash :results verbatim :exports both :session 807c02cd-a6cf-432f-b982-530fc81dbbf4 :cache yes
csm build
csm --action clean
#+END_SRC

#+RESULTS[]: csm-clean-aloud
: Building simulator
: action: remove build

* shipping the settings of your tool
  :PROPERTIES:
  :CUSTOM_ID: shipping-the-settings-of-your-tool
  :END:

The distribution can come with aliases of its own, when writing a command would
be too much. Put them in ~csm/settings/csm.json5~.

#+NAME: csm-distribution-settings
...
Like any other profile, it needs a version and may become obsolete. This
one-liner says the version clk is at, and don't forget to run it again when clk
moves on.

#+NAME: csm-settings-version
...
#+NAME: csm-run-shipped-alias
#+BEGIN_SRC bash :results verbatim :exports both :session 807c02cd-a6cf-432f-b982-530fc81dbbf4 :cache yes
csm go
#+END_SRC

#+RESULTS[]: csm-run-shipped-alias
: Generating code from model.xml
: Configuring build system
: Building simulator
: Running ./build/simulator
...
* the simulator crashes — time to debug
  :PROPERTIES:
  :CUSTOM_ID: the-simulator-crashes-time-to-debug
  :END:
...
* adding launcher support
  :PROPERTIES:
  :CUSTOM_ID: adding-launcher-support
  :END:
...
* using a launcher in the simulate command
  ...
  :CUSTOM_ID: using-a-launcher-in-the-simulate-command
  :END:
...
check-result(csm-fork-again)

check-result(csm-fork-force)
...
check-result(csm-configure-options)

check-result(csm-configure-defines)

check-result(csm-build-options)

check-result(csm-run-no-flow)

check-result(csm-clean-dry-run)

check-result(csm-clean-for-real)

check-result(csm-clean-aloud)

<<csm-ship-settings>>

check-result(csm-run-shipped-alias)
