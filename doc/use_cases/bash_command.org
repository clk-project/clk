...
#+SUBTITLE: Creating bash commands, clk_usage, clk_help_handler, source _clk.sh, exit codes, traps
...
* when clk cannot read what you wrote
  :PROPERTIES:
  :CUSTOM_ID: when-clk-cannot-read-what-you-wrote
  :END:

The lines under the ~--~ have a shape. Get one wrong and clk says which shape it
wanted.
...
#+BEGIN_SRC bash :exports none :tangle ../../tests/use_cases/bash_command.sh :noweb yes :shebang "#!/usr/bin/env bash"
  set -eu
<<init>>
