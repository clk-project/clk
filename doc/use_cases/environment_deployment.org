...
#+SUBTITLE: Auto-detecting deployment target from context (e.g. git branch) with explicit overrides, get_parameter_source
...
* A smart deployment command
  :PROPERTIES:
  :CUSTOM_ID: a-smart-deployment-command
  :END:
...
* Shortcut aliases for common targets
  :PROPERTIES:
  :CUSTOM_ID: shortcut-aliases-for-common-targets
  :END:
...
#+NAME: run
#+BEGIN_SRC bash :results none :exports none :tangle ../../tests/use_cases/environment_deployment.sh :noweb yes :shebang "#!/usr/bin/env bash"
  set -eu
<<init>>
