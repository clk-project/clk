...
#+RESULTS[]: try-completion-bad-command
: python
...
That last line promises a stacktrace, and ~clk --develop~ keeps the promise. It
is long, so let's only keep the line clk gave up on.
...
#+RESULTS[]: try-bad-command-develop
