...
* when the command breaks
  :PROPERTIES:
  :CUSTOM_ID: when-the-command-breaks
  :END:
...
Other days it breaks in a way nobody saw coming, and clk says as much rather
than pretending.
...
check-result(run-notyet)

<<write-boom>>

check-result(run-boom)
#+END_SRC
