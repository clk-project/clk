...
#+SUBTITLE: Monkey-patching clk internals with the plugin mechanism
...
** listing triggers with --name-only
   ...
   :ID:       653ccf6d-e530-4d30-9469-704a941c607f
   :END:
...
* writing one of your own
  :PROPERTIES:
  :CUSTOM_ID: writing-one-of-your-own
  :END:

Copying a file into the plugins directory works, but clk can lay it out for you,
in the profile you name.
...
#+NAME: ask_plugin_completion
#+BEGIN_SRC bash :results none :exports code
clk plugin which global g<TAB>
#+END_SRC

#+NAME: try_plugin_completion
#+BEGIN_SRC bash :results verbatim :exports results :session a197f802-e04c-40a2-8bfa-bc96ba703252 :cache yes
clk completion try --last plugin which global g
#+END_SRC

#+RESULTS[]: try_plugin_completion
: greet

* the day one stops working
  :PROPERTIES:
  :CUSTOM_ID: the-day-one-stops-working
  :END:

A plugin is python like any other, and it leans on what is installed. Remove the
library it wanted and it no longer loads.
...
#+NAME: run_with_broken_plugin
#+BEGIN_SRC bash :results verbatim :exports both :session a197f802-e04c-40a2-8bfa-bc96ba703252 :cache yes
clk echo hello
#+END_SRC

#+RESULTS[]: run_with_broken_plugin
...
#+BEGIN_SRC bash :exports none :tangle ../../tests/use_cases/using_a_plugin.sh :noweb yes :shebang "#!/usr/bin/env bash"
  set -eu
<<init>>
...
check-result(which_plugin)

check-result(create_plugin_again)

check-result(try_plugin_completion)

<<break_plugin>>

check-result(run_with_broken_plugin)
#+END_SRC
