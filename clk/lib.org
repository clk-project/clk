...
* format_options
  :PROPERTIES:
  :CUSTOM_ID: 0d4a3f2e-8b17-4c6a-9e35-1c7d2b4f8a60
  :END:

  A command that drives another program gathers its options as it goes, then
  hands the whole to ~call~. Gather them in a dictionary and this turns it into
  the list ~call~ expects.

  An option is named on the command line with dashes, whatever it is called in
  your code.
  ...
  A true one is the option alone, a false one says nothing at all, and a list
  says its option once per value. Some programs want the value glued to the
  option instead, hence ~glue~.
  ...
  See [[file:../doc/use_cases/chaotic_simulator_manager.org][chaotic simulator manager]] for a command handing its own options over this
  way.
