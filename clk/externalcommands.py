...
class ExternalCommandResolver(CommandResolver):
    ...
    def _get_command(self, path, parent=None):
        ...
        def compute_env(kwargs={}):
            env = {
                ("CLK___" + key).upper(): (value_to_string(value))
            ...
            env["PATH"] = (
                str(Path(__file__).parent / "commands/command")
                + ":"
                + os.environ["PATH"]
            )
            return env
            ...
            if process.returncode == 0:
                ...
                for line in metadata_out.splitlines():
                    ...
                    if line.startswith("A:"):
                        m = re.match(
                            "^A:(?P<name>[^:]+):(?P<type>[^:]+):(?P<help>[^:]+)(:(?P<extra>{.+}))?$",
                            line,
                        )
                        if m is None:
                            raise click.UsageError(
                    ...
                    if m is not None:
                        ...
                        if "I" in meta:
                            ignore_unknown_options = True
                    m = re.match(
                        "^flowoptions: *(?P<target>[^:]+):(?P<options>.+)$", line
                    )
                    if m is not None:
                        target = m.group("target")
                        cmd_flowoptions.append(
                            (target, m.group("options").replace(" ", "").split(","))
                        )
        ...
        if remaining_args:
            external_command = argument("args", nargs=-1, help=remaining_args)(
                external_command
            )
        for o in options:
            if "type" in o:
        ...
        if cmd_flowoptions:
            ...
            for target, options in cmd_flowoptions:
                ...
                external_command = flow_options(
                    options=options, target_command=target_command
                )(external_command)
