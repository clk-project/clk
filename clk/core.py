...
def resolve_ctx(cli, prog_name, args, resilient_parsing=True):
    ...
    while has_unparsed_args(ctx) and isinstance(ctx.command, click.Group):
...
def reproducible_output_callback(ctx, attr, value):
    if value:
        ...
        def _fake_sleep(seconds):
            ...
            faked_time_file = os.environ.get("CLK_FAKED_TIME_FILE")
            if faked_time_file:
                from datetime import datetime, timezone

                dt = datetime.fromtimestamp(_fake_clock[0], tz=timezone.utc)
                Path(faked_time_file).write_text(dt.isoformat())
