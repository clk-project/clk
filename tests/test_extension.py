...
def test_can_use_settings(lib):
    ...
    assert (
        lib.cmd("some-group-with-settings show some-key")
        == "['some', 'default', 'value']"
    )
    ...
    assert (
        lib.cmd("some-group-with-settings show --use-settings some-key")
        == "['some', 'list']"
    )
    assert (
        lib.cmd("some-group-with-settings show --use-settings2 some-key")
        == "['some', 'default', 'value', 'some', 'list']"
    )
    assert (
        lib.cmd("some-group-with-settings show another-key")
        == "['some', 'default', 'value']"
    )
    ...
    assert re.match(
        ".*invalid choice: another-key. .choose from some-key, some-other-key.*",
        e.value.stderr,
    )
